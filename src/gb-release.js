...
const {
	KEYS, MESSAGES, RELEASE_TYPES, BRANCHES, ENVS,
} = require('./data');
...
class GbRelease extends GbBase {
	...
	run() {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const statusData = await git.status();
				...
				const buildStrings = Object.keys(this.getData('builds')).map(build => `${build}@${semver.inc(this.clientPkg.version, this.releaseType)}`);
				const builds = this.parseBuildStrings(buildStrings);
	...
	doRelease() {
		return new Promise(async (resolve, reject) => {
			...
			git.push('origin', BRANCHES.PRODUCTION)
				.then(() => githubClient.repos.createRelease({
					owner: this.repoOwner,
					repo: this.repoName,
					tag_name: to,
					body: msg,
	 ...
	 */
	closePrs(prs) {
		return prs.map(pr => new Promise((resolve, reject) => {
